...
| AI 澄清 | 画一个那个东西 | 不确定时返回澄清原因，不直接乱画 |
...
| AI 澄清 | 已实现基础版 | AI 可返回 `unknown` 或 `clarify`，应用不会执行未经校验的内容 |
...
- Playwright 端到端测试通过 `?e2e=1` 测试入口模拟浏览器语音识别返回文本，并 mock `/api/ai/intent`，验证真实页面中的画布更新、AI 兜底解析、AI 回退诊断、目标编辑和移动端布局。
